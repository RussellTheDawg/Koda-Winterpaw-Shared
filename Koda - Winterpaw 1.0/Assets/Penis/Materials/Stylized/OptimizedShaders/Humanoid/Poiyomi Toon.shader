Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/5391fd42873437144a2e02b7b1c0dc5c"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if (_AudioLinkAnimToggle)
				{
					if (AudioLinkIsAvailable())
					{
						lineWidth += lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, AudioLinkData(uint2(0, (1.0 /*_AudioLinkOutlineSizeBand*/))));
					}
				}
				#endif
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + (0.25 /*_AlphaMod*/));
			...
			{
				float smoothness = (0.577 /*_MochieRoughnessMultiplier*/);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.occlusion = min(min(min(lerp(1, AOMaps.r, (0.75 /*_LightDataAOStrengthR*/)), lerp(1, AOMaps.g, (0.0 /*_LightDataAOStrengthG*/))), lerp(1, AOMaps.b, (0.0 /*_LightDataAOStrengthB*/))), lerp(1, AOMaps.a, (0.0 /*_LightDataAOStrengthA*/)));
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_OutlineClipAtZeroWidth*/))
				{
					...
					if (poiMods.audioLinkAvailable)
					{
						lineWidth += lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(1.0 /*_AudioLinkOutlineSizeBand*/)]);
					}
					#endif
				...
				float4 outlineColor = float4(0.7991028,0.7991028,0.02842603,1);
				#ifdef POI_AUDIOLINK
				if ((1.0 /*_OutlineALColorEnabled*/) && poiMods.audioLinkAvailable)
				{
					outlineColor = lerp(outlineColor, float4(0.7991028,0.7991028,0.02842603,1), smoothstep(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(0.0 /*_AudioLinkOutlineColorBand*/)]));
				}
				#endif
