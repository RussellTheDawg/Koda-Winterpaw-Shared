    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        float size = float(0.217);
        
        if(float(0))
        {
            size = remapClamped(randomFromPoint, 0, 1, float4(0.2255034,0.6255034,0,1).x, float4(0.2255034,0.6255034,0,1).y);
        }
        half glitterAlpha = 1;
        switch(float(0))
        {
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(754)), 0);
