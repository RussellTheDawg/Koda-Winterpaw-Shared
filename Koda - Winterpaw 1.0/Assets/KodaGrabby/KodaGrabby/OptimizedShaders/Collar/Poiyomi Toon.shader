Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/adf321ae560061949a87bc71a69c558f"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				if (_AudioLinkAnimToggle)
				{
					...
					{
						lineWidth += lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, AudioLinkData(uint2(0, (0.0 /*_AudioLinkOutlineSizeBand*/))));
					}
				}
				#endif
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if ((0 /*_ToonRampCount*/) > 1)
				{
					rampUVs.y = (floor(poiMesh.uv[(1 /*_ToonRampUVSelector*/)].y * (0 /*_ToonRampCount*/)) + 0.5) / (0 /*_ToonRampCount*/);
				}
				poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D_SAMPLER(_ToonRamp, _linear_clamp, rampUVs).rgb, shadowStrength);
				poiLight.finalLighting = lerp(float4(0,0,0,1) * lerp(poiLight.indirectColor, poiLight.rampedLightMap * poiLight.directColor, (1.0 /*_LightingIgnoreAmbientColor*/)) * poiLight.occlusion, poiLight.directColor, poiLight.rampedLightMap) * attenuation;
				#endif
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float metallic = (0.7 /*_MochieMetallicMultiplier*/);
				...
				#endif
				reflectionMask *= (0.9 /*_MochieReflectionStrength*/);
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				if ((1.0 /*_OutlineClipAtZeroWidth*/))
				{
					...
					if (poiMods.audioLinkAvailable)
					{
						lineWidth += lerp(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(0.0 /*_AudioLinkOutlineSizeBand*/)]);
					}
					#endif
				...
				{
					outlineColor = lerp(outlineColor, float4(0.7991028,0.7991028,0.028426,1), smoothstep(float4(0,1,0,0).x, float4(0,1,0,0).y, poiMods.audioLink[(0.0 /*_AudioLinkOutlineColorBand*/)]));
				}
				#endif
