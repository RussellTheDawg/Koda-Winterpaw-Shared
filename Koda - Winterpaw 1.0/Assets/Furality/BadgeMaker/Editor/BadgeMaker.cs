...
{	
    public class BadgeMaker : EditorWindow
    {
        ...
        private const string CurrentConvention = "Furality Somna";
        ...
        { 
            ...
            if (_badgeConvention == -1)
                _badgeConvention = _conventionNames.Contains(CurrentConvention) ? _conventionNames.IndexOf(CurrentConvention) : _conventionNames.Count - 1;
        ...
        private static MagickImage FindFontSize(string fontFamily, string text, int desiredWidth, int desiredHeight, MagickColor color)
        {
            ...
            try
            {
                image = new MagickImage($"label:{text}", new MagickReadSettings
                {
