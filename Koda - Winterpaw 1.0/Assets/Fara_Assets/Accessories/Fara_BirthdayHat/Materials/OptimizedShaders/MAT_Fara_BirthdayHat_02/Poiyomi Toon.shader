Shader "Hidden/Locked/.poiyomi/Old Versions/9.0/Poiyomi Toon/43bdd2a6a95b2ee4f8c6b8c5f13e2983"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float2 decalCenter = float4(0.5,0.51,1,1);
				float theta = radians((0.0 /*_ALUVRotation*/) + _Time.z * (0.0 /*_ALUVRotationSpeed*/));
				float cs = cos(theta);
				float sn = sin(theta);
				uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
				uv = remap(uv, float2(0, 0) - float4(1,1,1,1).xz / 2 + float4(0.5,0.51,1,1), float4(1,1,1,1).yw / 2 + float4(0.5,0.51,1,1), float2(0, 0), float2(1, 1));
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float metallic = (0.156 /*_MochieMetallicMultiplier*/);
