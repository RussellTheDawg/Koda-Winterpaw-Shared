Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/65d37d6652edec74bacd2031a78744df"
{
	Properties
	{
		...
		[sRGBWarning]_EmissionMask ("Emission Mask--{reference_properties:[_EmissionMaskPan, _EmissionMaskUV, _EmissionMaskInvert]}", 2D) = "white" { }
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(0) > 0)
				{
					emissionMask0 = customBlend(emissionMask0, poiMods.globalMask[float(0)-1], float(2));
				}
