Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/70ce1b120309ead448a6ed30bacce721"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.1.142</color>", Float) = 0
 ...
 #define PROP_DISSOLVENOISETEXTURE 
			...
			)
			{
				...
				if(float(0) && float(1))
				{
					...
					dissolveUdim += (v.uv0.xy * (float(0) == 0));
					dissolveUdim += (v.uv1.xy * (float(0) == 1));
					dissolveUdim += (v.uv2.xy * (float(0) == 2));
					dissolveUdim += (v.uv3.xy * (float(0) == 3));
					...
					isDiscardedFromDissolve += (dissolveUdim.y >= 0 && dissolveUdim.y < 1) * dot(float4(float(0)==1, float(0)==1, float(0)==1, float(0)==1), xMaskDissolve);
					isDiscardedFromDissolve += (dissolveUdim.y >= 1 && dissolveUdim.y < 2) * dot(float4(float(0)==1, float(0)==1, float(0)==1, float(0)==1), xMaskDissolve);
					isDiscardedFromDissolve += (dissolveUdim.y >= 2 && dissolveUdim.y < 3) * dot(float4(float(0)==1, float(0)==1, float(0)==1, float(0)==1), xMaskDissolve);
					isDiscardedFromDissolve += (dissolveUdim.y >= 3 && dissolveUdim.y < 4) * dot(float4(float(0)==1, float(0)==1, float(0)==1, float(0)==1), xMaskDissolve);
			...
			void calculateGlobalThemes(inout PoiMods poiMods)
			{
				...
				poiMods.globalColorTheme[0] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor0.rgb, float(0), float(0), float(0)), themeColorExposures.x), float4(1,1,1,1).a);
				poiMods.globalColorTheme[1] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor1.rgb, float(0), float(0), float(0)), themeColorExposures.y), float4(1,1,1,1).a);
				poiMods.globalColorTheme[2] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor2.rgb, float(0), float(0), float(0)), themeColorExposures.z), float4(1,1,1,1).a);
				poiMods.globalColorTheme[3] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor3.rgb, float(0), float(0), float(0)), themeColorExposures.w), float4(1,1,1,1).a);
			}
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				if (float(0) > 0)
				{
					dissolveMask = customBlend(dissolveMask, poiMods.globalMask[float(0)-1], float(2));
				}
				#if defined(PROP_DISSOLVETOTEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0.1,0.1,0,0)).r;
				#else
				...
				if(float(0))
				{
					float2 udim = floor(poiMesh.uv[(int)float(0)]);
					float4 xMask = float4(  (udim.x >= 0 && udim.x < 1),
					(udim.x >= 1 && udim.x < 2),
					(udim.x >= 2 && udim.x < 3),
					(udim.x >= 3 && udim.x < 4));
					da += (udim.y >= 0 && udim.y < 1) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
					da += (udim.y >= 1 && udim.y < 2) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
					da += (udim.y >= 2 && udim.y < 3) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
					da += (udim.y >= 3 && udim.y < 4) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
				}
				#ifdef POI_AUDIOLINK
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(2,2,0,0));
				#else
				...
				switch(float(1))
				{
					...
					{
						da = remap(da, 0, 1, -float(0.05), 1);
						dissolveAlpha = da;
						dds *= smoothstep(1, 0.99, da) * lerp(1, smoothstep(0, lerp(0.01, 0.1, dds), da), float(0));
						...
						edgeAlpha = remapClamped(da + float(0.05), da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						if(float(0))
						{
							da = remap(da, 1, 0, -float(0.05), 1);
						} else {
							da = remap(da, 0, 1, -float(0.05), 1);
						}
						dissolveAlpha = da;
						dds *= smoothstep(0, 0.2*dds + 0.01, dissolveAlpha) * lerp(1, smoothstep(1, 1 - 0.2*dds - 0.01, dissolveAlpha), float(0));
						float currentDistance = lerp(0, float(1.5), dissolveAlpha);
						float fragDistance = distance(float4(0,0,0,1), poiMesh.localPos.xyz);
						float normalizedDistance;
						normalizedDistance = (fragDistance - currentDistance) / (float(1.5) + 0.0001)  - dissolveDetailNoise * dds;
						if(float(0))
						{
							dissolveAlpha = (normalizedDistance > 0) ? 1 : 0;
							edgeAlpha = smoothstep(float(0.05) + .00001, 0, -normalizedDistance);
						} else {
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
							edgeAlpha = smoothstep(float(0.05) + .00001, 0, normalizedDistance);
						}
					...
					case 4: // CenterOut
					{
						...
						switch(float(1))
						{
							...
							{
								ramp = saturate(lerp(poiLight.vertexNDotV, poiLight.nDotV, float(0)));
								break;
							}
							case 2: // Custom Direction
							{
								ramp = dot(normalize(float4(0,0,1,0)), lerp(poiMesh.normals[0], poiMesh.normals[1], float(0)));
								ramp = saturate(ramp * .5 + 0.5);
								break;
							}
							case 3: // Light Direction
							{
								ramp = lerp(poiLight.vertexNDotL, poiLight.nDotL, float(0));
								ramp = saturate(ramp * .5 + 0.5);
								break;
							}
						}
				...
				if (float(0.05) || (float(1) == 2 && float(0.1) != 0))
				{
					...
					#endif
					poiFragData.baseColor = lerp(poiFragData.baseColor, edgeColor.rgb, smoothstep(0, 1 - float(1) * .99999999999, edgeAlpha));
				}
				poiFragData.emission += lerp(0, dissolveToTexture * float(1), dissolveAlpha) + lerp(0, edgeColor.rgb * float(2), smoothstep(0, 1 - float(1) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				indirectColor = lerp(indirectColor, indirectColor*poiFragData.baseColor, float(0));
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				#if defined(PROP_SKINTHICKNESS) || !defined(OPTIMIZER_ENABLED)
				float subsurface = 1 - POI2D_SAMPLER_PAN(_SkinThicknessMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				#else
				float subsurface = 1;
				#endif
				if(float(0))
				...
				if(float(1) != 1)
				{
					subsurface = pow(subsurface, float(1));
				}
				...
				poiLight.finalLighting = lerp(poiLight.directColor, min(lerp(poiLight.indirectColor * float4(1,1,1,1), float4(1,1,1,1), float(0)) * poiLight.occlusion + (sss * poiLight.directColor), poiLight.directColor), float(1));
				#endif
				#ifdef _LIGHTINGMODE_SDF
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, float(0), float(0), _MochieReflCube, _MochieReflCube_HDR, poiCam.reflectionDir);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.occlusion = min(min(min(lerp(1, AOMaps.r, float(1)), lerp(1, AOMaps.g, float(0))), lerp(1, AOMaps.b, float(0))),lerp(1, AOMaps.a, float(0)));
				#else
				...
				for (int index = 0; index < 4; index++)
				{
					...
					poiLight.vColor[index] = lerp(unity_LightColor[index].rgb, dot(unity_LightColor[index].rgb, float3(0.299, 0.587, 0.114)), float(0));
